...
export class ToastService {
  ...
  public determineToast(response: ServerResponse<any>, description?: string): void {
    response.code == 200 ? 
      this.showToast("SUCCESS_TOAST", description ? description : response.message, "alert-success", "", 4000)
      : this.showToast("EROOR_TOAST", description ? description : response.message, "alert-danger", "", 4000);
  }
